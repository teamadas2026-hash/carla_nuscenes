...
class Client:
    ...
    def generate_world(self,world_config):
        ...
        # Map CARLA blueprint ids to model class ids.
        ...
        def map_bp_to_class(bp_id):
            bp_id = bp_id.lower()
            if bp_id.startswith("walker"):
                return "pedestrian"
            if bp_id.startswith("vehicle"):
                if any(k in bp_id for k in ["truck", "carlacola", "cybertruck"]):
                    return "truck"
                if any(k in bp_id for k in ["bus", "coach"]):
                    return "bus"
                if "trailer" in bp_id:
                    return "trailer"
                if any(k in bp_id for k in ["construction", "bulldozer", "excavator"]):
                    return "construction_vehicle"
                if any(k in bp_id for k in ["motorcycle", "motorbike"]):
                    return "motorcycle"
                if "bicycle" in bp_id or ".bike" in bp_id:
                    return "bicycle"
                return "car"
            if any(k in bp_id for k in ["trafficcone", "traffic_cone", "cone"]):
                return "traffic_cone"
            if "barrier" in bp_id:
                return "barrier"
            return None

        def class_to_nuscenes_category(cls_name):
            if cls_name in ["car", "truck", "bus", "trailer", "construction_vehicle", "motorcycle", "bicycle"]:
                return f"vehicle.{cls_name}"
        ...
        self.category_dict = {}
        self.model_class_id_by_bp = {}
        for bp in self.world.get_blueprint_library():
            cls_name = map_bp_to_class(bp.id)
            self.category_dict[bp.id] = class_to_nuscenes_category(cls_name)
            self.model_class_id_by_bp[bp.id] = self.model_class_ids.get(cls_name)
    ...
    def get_calibrated_sensor(self, sensor):
        ...
        if sensor.bp_name == "sensor.camera.rgb":
            intrinsic = get_intrinsic(float(sensor.get_actor().attributes["fov"]),
                            float(sensor.get_actor().attributes["image_size_x"]),
                            float(sensor.get_actor().attributes["image_size_y"])).tolist()
            rotation, translation = get_nuscenes_rt(sensor.transform)        # ← FIXED: no mode
        elif sensor.bp_name == "sensor.lidar.ray_cast":
            intrinsic = []
            rotation, translation = get_nuscenes_rt(sensor.transform, "zxy") # ← FIXED: zxy for LiDAR only
        else:
            intrinsic = []
            rotation, translation = get_nuscenes_rt(sensor.transform)        # radar/gnss/imu
        return sensor_token, channel, translation, rotation, intrinsic
        """
